...
- **Persistence:** profile lives in `calibration/` (gitignored), persisted between sessions; include a version field so format changes are detectable. Reuse LeRobot's calibration facility where it fits, but the *hand-eye image-Jacobian* part is Dum-E-specific (LeRobot's own calibration is about joint zeroing/ranges; the joint→pixel mapping is ours).
